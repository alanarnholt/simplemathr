...
#' @param tquan type of quantile 1 through 9.  For the various types of quantiles, see \code{\link{quantile}}.
#' 
...
#' @keywords hplot
#######################################################################
eda <- function(x, trim = 0.05, dec = 5, tquan = 7)
{
  ...
  x <- x[is.finite(x)]
  n <- length(x)
  Q1 <- round(quantile(x, type = tquan)[2], digits = dec)
  Q3 <- round(quantile(x, type = tquan)[4], digits = dec)
